...
const toml_value = @import("../value.zig");
const TomlValue = toml_value.Value;
const TomlValueList = toml_value.ValueList;
const TomlTable = @import("../table.zig").Table;
...
fn serializeValue(state: *SerializerState, t: std.builtin.Type, value: anytype, writer: *std.Io.Writer) anyerror!void {
    switch (t) {
                ...
                switch (t.pointer.size) {
                    ...
                    .slice => {
                        ...
                        if (value.len != 0) {
                            ...
                            while (i < value.len - 1) {
                                const elm = value[i];
                                try serializeValue(state, @typeInfo(t.pointer.child), elm, writer);
                                try writer.print(", ", .{});
                                i += 1;
                            }
                            const elm = value[value.len - 1];
                            try serializeValue(state, @typeInfo(t.pointer.child), elm, writer);
                        }
                        try writer.print(" ]", .{});
                    },
...
fn serializeMap(state: *SerializerState, value: anytype, writer: *std.Io.Writer) !void {
    {
        ...
        const first_key = key_iter.next() orelse return;
        const key_type = @typeInfo(@TypeOf(first_key.*));
    ...
    const first_value = value_iter.next() orelse return;
    const value_type = @TypeOf(first_value.*);
    ...
    if (@TypeOf(value.get(fields[0]).?) == TomlValue) {
        if (state.table_comp.items.len == 0) {
            // Root table: emit scalars/inline arrays first, then tables, then array-of-tables.
            for (fields) |field| {
                const entry = value.get(field).?;
                switch (entry) {
                    .table => {},
                    .array => |ar| {
                        if (arrayAllTables(ar)) {
                            continue;
                        }
                        try serializeTomlEntry(state, field, entry, writer);
                    },
                    else => try serializeTomlEntry(state, field, entry, writer),
                }
            }
            for (fields) |field| {
                const entry = value.get(field).?;
            ...
            for (fields) |field| {
                const entry = value.get(field).?;
                if (entry == .array and arrayAllTables(entry.array)) {
                    try serializeTomlEntry(state, field, entry, writer);
                }
            }
        } else {
...
fn serializeTomlEntry(state: *SerializerState, key: []const u8, value: TomlValue, writer: *std.Io.Writer) anyerror!void {
    switch (value) {
        .table => |t| {
            if (state.table_comp.items.len == 0) {
                try writer.print("[{s}]\n", .{key});
                try state.table_comp.append(state.allocator, key);
                try serializeMap(state, t.*, writer);
                _ = state.table_comp.pop();
            } else {
                try writer.print("{s} = ", .{key});
                try serializeInlineTable(state, t.*, writer);
                _ = try writer.write("\n");
            }
        },
        .array => |ar| {
            if (state.table_comp.items.len == 0 and arrayAllTables(ar)) {
                for (ar.items) |item| {
                    switch (item) {
                        .table => |t| {
                            try writer.print("[[{s}]]\n", .{key});
                ...
                try writer.print("{s} = ", .{key});
                try serializeTomlValueInline(state, value, writer);
                _ = try writer.write("\n");
            }
        },
        else => {
...
fn serializeTomlValueInline(state: *SerializerState, value: TomlValue, writer: *std.Io.Writer) anyerror!void {
    switch (value) {
        .string => |s| try writer.print("\"{s}\"", .{s}),
        .integer => |i| try writer.print("{}", .{i}),
        .float => |f| try writer.print("{}", .{f}),
        .boolean => |b| try writer.print("{s}", .{if (b) "true" else "false"}),
        .date => |d| try serializeStruct(state, d, writer),
        .time => |t| try serializeStruct(state, t, writer),
        .datetime => |dt| try serializeStruct(state, dt, writer),
        .array => |ar| {
            try writer.print("[ ", .{});
            for (ar.items, 0..) |item, idx| {
                if (idx > 0) try writer.print(", ", .{});
                try serializeTomlValueInline(state, item, writer);
            }
            try writer.print(" ]", .{});
        },
        .table => |t| try serializeInlineTable(state, t.*, writer),
    }
}

fn serializeInlineTable(state: *SerializerState, table: TomlTable, writer: *std.Io.Writer) anyerror!void {
    var key_iter = table.keyIterator();
    const key_type = @typeInfo(@TypeOf(key_iter.next().?.*));
    if (!hasStringType(key_type)) @panic("Maps with non-string compatible types cannot be serialized");

    var fields = try state.allocator.alloc([]const u8, table.count());
    defer state.allocator.free(fields);

    var counter: u32 = 0;
    var key_it = table.keyIterator();
    ...
    for (fields, 0..) |field, idx| {
        if (idx > 0) try writer.print(", ", .{});
        try writer.print("{s} = ", .{field});
        try serializeTomlValueInline(state, table.get(field).?, writer);
    }
    try writer.print(" }}", .{});
}

fn arrayAllTables(ar: *TomlValueList) bool {
    if (ar.items.len == 0) return false;
    for (ar.items) |item| {
        switch (item) {
            .table => {},
            else => return false,
